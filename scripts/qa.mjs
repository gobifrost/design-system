...
async function desktopContract() {
  ...
  const tableOverflow = await opportunities.locator("xpath=..").evaluate((scroller) => scroller.scrollWidth - scroller.clientWidth);
  if (tableOverflow > 1) failures.push(`data: fitting table manufactured ${tableOverflow}px of horizontal overflow`);
  await page.getByRole("button", { name: "Actions for Security uplift" }).click();
  const rowMenu = page.getByRole("menu", { name: "Actions for Security uplift" });
