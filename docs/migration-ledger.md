...
| Fields | `ui/input.tsx:5`, `label.tsx:6`, `select.tsx:7`, `textarea.tsx:5`, `form.tsx:19` | BfField anatomy + compatible low-level primitives | Preserve RHF/Radix/native APIs | Phase 1 field ready; adapter/composition pending |
...
| Data | `ui/data-table.tsx:38`, `table.tsx:5`, repeated entity tables | BfDataTable + list/table recipes | Preserve semantic table and row navigation | Inventoried; reinvention planned |
...
3. Standardize state frames, async operation progress, entity list/table surfaces, and dialog forms.
...
- Whether DataTable row navigation standardizes on links or callbacks. Recommendation: semantic link when a URL exists; callback/button only for true commands.
