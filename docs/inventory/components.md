...
| BfDataTable | `client/src/components/ui/data-table.tsx:38`, `table.tsx:5`; `EventsTable.tsx:176`, `Knowledge.tsx:423`, `LogsTable.tsx:62` | Semantic wrappers, owned scroller/pinned footer, modifier/middle-click row URLs; consumers add sorting, selection, states, filters, pagination | 34 / 8 | Production typed source now consolidates sorting, selection, loading/empty/error, toolbars, sticky headers, row activation, and pinned pagination. Preserve unprefixed product exports through adapters. |
...
| BfSearch / Command / Combobox | `client/src/components/search/SearchBox.tsx:14`, `ui/command.tsx:60`, `ui/combobox.tsx:39`, `ui/multi-combobox.tsx:35` | SearchBox used 20 times; single/multi options/value/loading/empty APIs; cmdk filtering | 20 / 18 / 16 | BfCombobox and BfMultiSelect source now cover searchable set selection, descriptions/keywords, loading/disabled/error/empty, tags, limits, and bulk actions. Keep free search and command execution as distinct semantics. |
...
| BfOverlay menu | `client/src/components/ui/popover.tsx:6`, `dropdown-menu.tsx:9`, `context-menu.tsx:7`, `sheet.tsx:8`, `tooltip.tsx:19` | Radix compositions | 25 / 9 / 8 / 10 / 20 | BfActionMenu now covers viewport-aware local row actions, keyboard traversal, destructive tone, and focus return. Preserve the broader Radix submenu/modality APIs through adapters. |
